...
const Harness = () => {
  ...
  const secondLater = () => {
    lastId.current = alert.showAlert({ message: 'Second Later', type: ALERT_TYPES.WARNING });
  };

  const successWithConfirm = () => {
  ...
  return (
    <div>
      ...
      <button data-cy="btn-second-later" onClick={secondLater}>SecondLater</button>
      ...
      <button data-cy="btn-success-confirm" onClick={successWithConfirm}>SuccessWithConfirm</button>
...
describe('CustomAlert (Component)', () => {
  ...
  it('renders nothing when no alerts present initially', () => {
    mountWithProvider();
  ...
  it('renders info with title and closes on overlay click', () => {
    ...
    cy.get('.alert-overlay').click('topLeft', { force: true });
    cy.get('.alert-modal').should('not.exist');
  });
  ...
  it('success alert shows success icon and calls onConfirm without cancel button', () => {
    mountWithProvider();
    cy.get('[data-cy="btn-success-confirm"]').click();
    cy.get('.alert-modal.success .alert-icon.success-icon').should('exist');
    cy.get('.alert-btn-cancel').should('not.exist');
    cy.get('.alert-btn-confirm').click();
    cy.get('[data-cy="confirm-result"]').should('contain.text', 'ok');
  });

  it('only latest alert is displayed when multiple are pushed (separated >50ms)', () => {
    mountWithProvider();
    // First alert
    cy.get('[data-cy="btn-info"]').click();
    cy.get('.alert-modal .alert-message', { timeout: 8000 }).should('contain.text', 'Info body');
    // Wait beyond debounce window, then push another alert
    cy.wait(60);
    // Close the first alert via overlay to avoid overlay covering next buttons
    cy.get('.alert-overlay').click('topLeft', { force: true });
    // Push a new alert explicitly via dedicated button
    cy.get('[data-cy="btn-second-later"]').click({ force: true });
    cy.get('.alert-modal .alert-message', { timeout: 8000 }).should('contain.text', 'Second Later');
    cy.get('.alert-modal .alert-message').should('not.contain.text', 'Info body');
  });
  ...
  it('debounce prevents second alert within 50ms', () => {
    ...
    cy.get('.alert-modal .alert-message', { timeout: 8000 }).should('contain.text', 'Debounce A');
  });

  it('warning with autoClose removes itself after duration', () => {
    mountWithProvider();
    cy.get('[data-cy="btn-autoclose"]').click();
    cy.get('.alert-modal.warning', { timeout: 8000 }).should('exist');
    cy.wait(600);
    cy.get('.alert-modal').should('not.exist');
  });
  ...
  it('removeLast hides current alert when programmatically removed', () => {
    ...
    cy.get('[data-cy="btn-remove"]').click({ force: true });
    cy.get('.alert-modal').should('not.exist');
  });
});
